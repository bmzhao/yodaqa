...
/**
 * A wrapper around custom rdf dataset that maps concepts/entities to
 * list of entity properties. This can then serve as an
 * information source.
 */

public class CustomProperties extends CustomLookup {
	private static final Log logger =
			LogFactory.getLog(CustomProperties.class);
...
//		title = super.capitalizeTitle(title);
		...
		String rawQueryStr =
				...
				"FILTER ( !regex(str(?res), '^" + System.getProperty("customkbdomain") + "[^_]*:', 'i') )\n" +
				//todo change this with something more rigorous (config, command line option, and rename)
		...
		for (Literal[] rawResult : rawResults) {
			...
			logger.debug("Custom KB {} rawproperty: {} -> {} ({})", title, propLabel, value, valRes);
			AnswerFV fv = new AnswerFV();
			fv.setFeature(AF.OriginCustomProperty, 1.0);
